...
public final class IdempotencyTokenService {
    ...
    public boolean acquire(long userId, String token) {
        return Boolean.TRUE.equals(redis.opsForValue().setIfAbsent("order:idempotency:create:" + userId + ":" + token,
            "PROCESSING", Duration.ofMinutes(10)));
    }

    public void complete(long userId, String token, String orderNo) {
        redis.opsForValue().set("order:idempotency:create:" + userId + ":" + token, orderNo, Duration.ofMinutes(10));
    }
