...
public class OrderController {
  ...
  ApiResponse<OrderDtos.SettlementView> settlement(
      @Valid @RequestBody OrderDtos.SettlementRequest r) {
    return ApiResponse.ok(s.settle(r.items()), MDC.get("traceId"));
  }

  @PostMapping
  ApiResponse<OrderDtos.OrderView> create(
      @RequestHeader("X-User-Id") long u,
      @RequestHeader("X-Idempotency-Token") String t,
      @Valid @RequestBody OrderDtos.CreateRequest r) {
    return ApiResponse.ok(s.create(u, t, r.items()), MDC.get("traceId"));
  }

  @GetMapping
  ApiResponse<java.util.List<OrderDtos.OrderView>> list(
      @RequestHeader("X-User-Id") long u,
  ...
  @GetMapping("/{no}")
  ApiResponse<OrderDtos.OrderView> get(
      @PathVariable String no, @RequestHeader("X-User-Id") long u) {
