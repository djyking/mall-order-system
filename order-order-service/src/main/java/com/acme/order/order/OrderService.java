...
public class OrderService {
        ...
        items.stream()
            .map(
                i -> {
                  ...
                  return new OrderDtos.SettlementLine(
                      sku.skuId(), sku.skuName(), sku.priceCent(), i.quantity(), amount);
                })
            .toList();
    return new OrderDtos.SettlementView(
        lines, lines.stream().mapToLong(OrderDtos.SettlementLine::amountCent).sum());
  }
  ...
  public OrderDtos.OrderView create(long user, String token, List<OrderDtos.Item> requested) {
    ...
    try {
      ...
      remote.reserve(
          no, user, requested.stream().map(i -> new Line(i.skuId(), i.quantity())).toList());
      reserved = true;
  ...
  public void cancel(String no, long user, String reason) {
    var s = repo.get(no, user);
    if (s.status() == OrderStatus.CANCELED) return;
  ...
  public void deliver(String no, long user) {
    transit(
        no, user, OrderStatus.WAIT_DELIVERY, OrderStatus.WAIT_RECEIVE, "DELIVER", "OrderDelivered");
  }
  ...
  private void transit(
      String no, long user, OrderStatus expected, OrderStatus target, String op, String event) {
    var snap = repo.get(no, user);
    if (snap.status() != expected) machine.require(snap.status(), target);
  ...
  public void paymentSucceeded(String no, long user) {
    var s = repo.get(no, user);
    if (s.status() == OrderStatus.WAIT_DELIVERY) return;
    repo.markPaid(s);
  }
