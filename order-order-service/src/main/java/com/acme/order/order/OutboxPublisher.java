...
public class OutboxPublisher {
    ...
    public void publish() {
        var rows = jdbc.query(
            "SELECT id,event_id,event_type,aggregate_id,payload FROM outbox_event "
                + "WHERE event_status IN (0,20) AND next_retry_time<=NOW(3) ORDER BY id LIMIT 100",
        ...
        for (var e : rows) {
            try {
                ...
                jdbc.update(
                    "UPDATE outbox_event SET event_status=20,retry_count=retry_count+1,"
                        + "next_retry_time=DATE_ADD(NOW(),INTERVAL LEAST(300,POW(2,retry_count)) SECOND),"
