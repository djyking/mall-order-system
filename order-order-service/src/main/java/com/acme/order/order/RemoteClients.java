...
public class RemoteClients {
    ...
    public void reserve(String no, long user, List<Line> lines) {
        inventory.post().uri("/internal/inventory/reserve").body(new ReserveRequest(no, user, lines)).retrieve()
            .toBodilessEntity();
    }

    public void release(String no) {
        inventory.post().uri("/internal/inventory/release").body(new ChangeRequest(no)).retrieve().toBodilessEntity();
    }

    public PaymentDtos.PayView createPayment(String no, long user, long amount) {
        return payment.post().uri("/internal/payments").body(new PaymentDtos.CreateRequest(no, user, amount)).retrieve()
