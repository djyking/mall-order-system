...
public class RemoteClients {
  ...
  public RemoteClients(
      @Qualifier("productClient") RestClient p,
      @Qualifier("inventoryClient") RestClient i,
  ...
  public void reserve(String no, long user, List<Line> lines) {
    inventory
        ...
        .body(new ReserveRequest(no, user, lines))
        .retrieve()
        .toBodilessEntity();
  }
  ...
  public PaymentDtos.PayView createPayment(String no, long user, long amount) {
    return payment
        ...
        .body(new PaymentDtos.CreateRequest(no, user, amount))
        .retrieve()
