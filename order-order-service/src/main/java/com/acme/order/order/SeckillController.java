...
public class SeckillController {
  ...
  @PostMapping("/{sku}")
  ApiResponse<Map<String, String>> reserve(
    ...
    Boolean first =
        redis
            .opsForValue()
            .setIfAbsent(
                "order:seckill:user:" + sku + ":" + user,
                request,
                java.time.Duration.ofMinutes(10));
    String status;
    if (!Boolean.TRUE.equals(first)) status = "DUPLICATE";
    else {
      Long ok =
          redis.execute(
              script, List.of("order:stock:available:" + sku), Integer.toString(quantity));
      status = ok != null && ok == 1 ? "QUEUING" : "SOLD_OUT";
      if (!"QUEUING".equals(status)) redis.delete("order:seckill:user:" + sku + ":" + user);
    }
    redis
        .opsForValue()
        .set("order:seckill:result:" + request, status, java.time.Duration.ofHours(1));
  ...
  ApiResponse<Map<String, String>> result(@PathVariable String id) {
    ...
    return ApiResponse.ok(
        Map.of("requestId", id, "status", status == null ? "NOT_FOUND" : status),
        MDC.get("traceId"));
  }
}
