...
public class OrderProjectionConsumer {
    ...
    public void consume(JsonNode e) {
        String id = e.path("eventId").asText(), type = e.path("eventType").asText(),
            no = e.path("payload").path("orderNo").asText();
        ...
        jdbc.update(
            "INSERT INTO order_query_projection(order_no,user_id,status,pay_status,total_amount_cent,event_version,create_time,update_time) VALUES(?,?,?,'UNPAID',?,1,NOW(3),NOW(3)) ON DUPLICATE KEY UPDATE status=VALUES(status),pay_status=IF(VALUES(status)='WAIT_DELIVERY','PAID',pay_status),event_version=event_version+1,update_time=NOW(3)",
